...
class MultiMedia_Main_app(PyQt5.QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.equalizationPart.clicked.connect(self.go_equalizationPart)
        ...
        self.homework.clicked.connect(self.homework1)
    ...
    def image_view(self,file_name):
        ...
        try:
            self.input_image = io.imread(file_name)
    ...
    def go_equalizationPart(self):

        from equalization_window.eq_view import equalizationWindow
        # print('start')
        self.hide()
        self.equalization_window = equalizationWindow()  #
        self.equalization_window.exec()  # 두번째 창 닫을때 까지 기다림
        self.show()  # 두번째창 닫으면 다시 메인 창 보여짐
    ...
    def go_rotatePart(self):
        ...
        # print('start')
        self.hide()
        self.rotate_window = rotateWindow()  #
    ...
    def homework1(self):
        ...
        blur3 = cv2.GaussianBlur(self.input_image, (3, 3),3)
        temp_img= cv2.rotate(blur3, cv2.ROTATE_180)

        result = cv2.cvtColor(temp_img, cv2.COLOR_BGR2GRAY)
